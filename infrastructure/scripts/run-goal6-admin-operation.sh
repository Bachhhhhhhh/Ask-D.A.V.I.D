...
fail_closed() {
  printf 'Goal 6 admin operation stopped: %s\n' "$1" >&2
  exit 1
}
...
confirm_goal6_rebuild="false"
confirm_goal6_audit="false"
application_subnet_ids=()

while (($# > 0)); do
  case "$1" in
    --operation) operation="${2:-}"; shift 2 ;;
    --expected-query-id) expected_query_id="${2:-}"; shift 2 ;;
    ...
    --confirm-goal6-rebuild) confirm_goal6_rebuild="true"; shift ;;
    --confirm-goal6-audit) confirm_goal6_audit="true"; shift ;;
...
case "$operation" in
  rebuild)
    [[ "$confirm_goal6_rebuild" == "true" ]] || fail_closed "rebuild requires its explicit confirmation switch"
    [[ "$confirm_goal6_audit" == "false" ]] || fail_closed "audit confirmation is invalid for rebuild"
    [[ -z "$expected_query_id" ]] || fail_closed "rebuild must not receive a query ID"
    runner_command="/app/doris-rebuild-serving"
    marker_operation="rebuild-serving"
    ;;
  audit)
    [[ "$confirm_goal6_audit" == "true" ]] || fail_closed "audit requires its explicit confirmation switch"
    [[ "$confirm_goal6_rebuild" == "false" ]] || fail_closed "rebuild confirmation is invalid for audit"
    [[ "$expected_query_id" =~ ^[0-9A-Fa-f-]+$ ]] || fail_closed "audit query ID is invalid"
    runner_command="/app/doris-audit-verify"
    marker_operation="audit-verify"
    ;;
  *) fail_closed "operation must be rebuild or audit" ;;
esac
...
if [[ "$operation" == "audit" ]]; then
  overrides="$(jq -cn \
    ...
    --arg query_id "$expected_query_id" \
    '{containerOverrides:[{name:"doris-admin",command:[$command],environment:[{name:"EXPECTED_QUERY_ID",value:$query_id}]}]}')"
else
  overrides="$(jq -cn --arg command "$runner_command" \
    '{containerOverrides:[{name:"doris-admin",command:[$command]}]}')"
fi
...
run_json="$(aws ecs run-task \
  ...
  --output json)" || fail_closed "ECS rejected the one approved admin operation; no retry is performed"
...
' <<<"$task_json" >/dev/null || fail_closed "admin operation failed; inspect its CloudWatch stream and do not retry"
