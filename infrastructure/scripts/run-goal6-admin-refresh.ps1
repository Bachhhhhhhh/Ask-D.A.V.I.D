...
param(
    ...
    [switch]$ConfirmGoal6AdminRefresh
)
...
function Fail-Closed {
    ...
    throw "Goal 6 admin-refresh stopped: $Message"
}

if (-not $ConfirmGoal6AdminRefresh) {
    Fail-Closed "the explicit ConfirmGoal6AdminRefresh approval switch is required."
}
...
if ($definition.family -ne "$NamePrefix-doris-admin-refresh") {
    Fail-Closed "task-definition family is not the Goal 6 admin-refresh family."
}
...
if ($definition.containerDefinitions.Count -ne 1) {
    Fail-Closed "admin-refresh task must contain exactly one container."
}
...
if ($task.lastStatus -ne "STOPPED" -or $null -eq $containerResult -or $containerResult.exitCode -ne 0) {
    Fail-Closed "admin-refresh task failed. Inspect its CloudWatch stream; no retry is performed."
}
...
# CloudWatch inspection is metadata-only so secrets or task output are never printed.
...
$completionEvent = $events | Where-Object { $_ -match '"operation":"admin-refresh".*"status":"completed"' } | Select-Object -Last 1
if ([string]::IsNullOrWhiteSpace($completionEvent)) {
    Fail-Closed "CloudWatch evidence did not contain the non-sensitive completed status. No retry is performed."
}
